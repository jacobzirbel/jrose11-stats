...
export default async function Home() {
  ...
  const { data: runs, error } = await supabase
    .from('runs')
    .select('pokemon_id, youtube_url, status, pokemon(dex_number, name, sprite_url, type1, type2, is_glitch)')
    .order('pokemon_id')
        ...
        {typedRuns.map((run) => {
          ...
          return (
            ...
            >
              ...
              <div className="text-[10px] text-gray-400 truncate">{dexStr}</div>
              {run.youtube_url && (
